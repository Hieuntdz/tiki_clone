...
class ShockPriceWidget extends GetView<HomeController> {
  ...
  Widget timeText(String value) {
    return Container(
      ...
      child: Text(
        value,
        style: TextStyle(
            color: Colors.white, fontSize: AppDimens.defaultSmallTextSize),
      ),
    );
  }
  ...
  Widget listShockItem() {
    return Obx(
      () => ListView.separated(
        itemCount: controller.listShockPriceData.length > 0
            ? controller.listShockPriceData.length + 1
            : 0,
  ...
  Widget itemShockPriceData(ShockPriceData priceData) {
    ...
    int discount = (priceData.product.discount /
            (priceData.product.discount + priceData.product.price) *
            100)
        .round();
    return Container(
      width: 80,
        ...
        children: [
          ...
          CustomPaint(
            painter: ProgressPriceWidget(
                percent: (priceData.progress.orderedPercent / 100),
